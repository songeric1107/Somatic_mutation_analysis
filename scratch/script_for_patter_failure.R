...
all_matrix=read.table("/Users/ysong/Desktop/pattern_failure/result_417/onco_matrix.txt",sep="\t",row.names=1,check.names=F,header=T)

sub.matrix=all_matrix[,which(colnames(all_matrix)%in%meta.s[which(meta.s$Time..2.synchronous..1.metachronous==1),Tumor_Sample_Barcode])]
